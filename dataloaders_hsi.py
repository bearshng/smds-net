...
def get_dataloaders(train_path_list, val_path_list, crop_size=96, batch_size=1, downscale=0,
    ...
    tfs = []
    scal_ratio=np.arange(0.6,1.2,0.2)
    tfs += [
    MyResize(scal_ratio, crop_size),
    ...
    image_datasets = {'train': train,
                      'val': Dataset(val_path_list, data_transforms['val'], verbose=verbose, grey=grey)}
